...
def export(path, objects, main_column):
    path.mkdir(exist_ok=True)
    summary_path = path / 'summary.csv'
    with summary_path.open('w') as summary_file:
        summary_csv = DictWriter(summary_file, fieldnames=[main_column, *keywords])
        summary_csv.writeheader()
        for obj in objects:
            detail_path = path / f"{obj.code}.txt"
            summary = defaultdict(int)
            summary[main_column] = obj.code
...
export(programme_path, programme_file.programmes(), 'programme')
